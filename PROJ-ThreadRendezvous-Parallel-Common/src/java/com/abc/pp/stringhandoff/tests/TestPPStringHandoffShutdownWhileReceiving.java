...
public class TestPPStringHandoffShutdownWhileReceiving extends TestPPStringHandoffBase {
    public TestPPStringHandoffShutdownWhileReceiving(StringHandoffFactory factory,
        ...
        super("shtudown() called while receiving", new BasicScoringInfo(2, 14), factory, threadFactory);
    }
    ...
    protected void performTests() {
        try {
            outln("====================");
            launch((launcher) -> testShutdownCalledWhileReceiving(launcher));
            outln("====================");
            launch((launcher) -> testShutdownCalledWhileReceivingWithTimeout(launcher));
            outln("====================");
    ...
    private void testShutdownCalledWhileReceiving(Launcher launcher) throws InterruptedException {
        outln("-- shutdown while receiving, no timeout --");
    ...
    private void testShutdownCalledWhileReceivingWithTimeout(Launcher launcher) throws InterruptedException {
        outln("-- shutdown while receiving with timeout --");
        StringHandoff handoff = createDS();
